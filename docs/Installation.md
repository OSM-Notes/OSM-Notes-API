...
# Use a different port (e.g. when 3000 is in use) — or set `PORT` in `.env`
PORT=3001 npm start
...
The API will be available at `http://localhost:3000` unless you set another `PORT` in `.env` or on the command line.
